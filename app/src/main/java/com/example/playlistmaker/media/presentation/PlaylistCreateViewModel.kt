...
class PlaylistCreateViewModel(
    ...
    fun handleTitleChange(text: String) {
        val oldState = statePlaylistCreate.value
        if (!text.isEmpty()) {
            val updateState = PlaylistCreateState(
                text, oldState?.description,
                oldState?.filePath,true)
    ...
    fun createPlaylist() {
        viewModelScope.launch (Dispatchers.IO) {
            ...
            val newPlaylist = Playlist(null,curState?.title, curState?.description,
